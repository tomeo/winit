...
* configure-claude-plugins.ps1 installs Claude Code plugins from Anthropic's official marketplace in user scope: pyright-lsp and typescript-lsp (type errors, go-to-definition and find-references for the agent) and microsoft-docs (Microsoft Learn, including Graph). The two LSP plugins only wire the language servers up; install-nodejs.ps1 installs pyright and typescript-language-server themselves. An installed plugin is left alone, -Remove uninstalls them.

...
./scripts/configure-claude-skills.ps1
./scripts/configure-claude-plugins.ps1
...
* configure-claude-skills.ps1 installs third-party agent skills for Claude Code in user scope (~\.claude\skills) with `npx skills add`. Today that is herdr's own skill, which lets the agent drive herdr panes from inside herdr; it only triggers when herdr is mentioned and does nothing outside a herdr pane. It installs with --copy because symlinks on Windows need Developer Mode or elevation. It runs after install-nodejs.ps1, which provides npx. An installed skill is left alone, -Update pulls the latest version and -Remove takes it out again.
* configure-claude-plugins.ps1 installs Claude Code plugins from Anthropic's official marketplace in user scope: pyright-lsp and typescript-lsp (type errors, go-to-definition and find-references for the agent), microsoft-docs (Microsoft Learn, including Graph) and hookify (turns "never do X" rules into hooks that block the call). The two LSP plugins only wire the language servers up; install-nodejs.ps1 installs pyright and typescript-language-server themselves. An installed plugin is left alone, -Remove uninstalls them.
...
* set-touchpad-gestures.ps1 sets the three finger swipes and taps to Nothing, so a third finger landing during a two finger scroll no longer throws up Task View. Two finger scrolling, zoom and taps are left alone. -IncludeFourFinger turns off the four finger gestures as well, which is how init.ps1 runs it, and -Revert puts the Windows defaults back. The values are per user and live in HKCU, so no elevation and no effect on other accounts.
